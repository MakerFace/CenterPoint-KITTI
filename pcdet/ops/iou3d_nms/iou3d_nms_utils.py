...
def box_iou3d_gpu(boxes_a, boxes_b):
    """
    Args:
        boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (N, 7) [x, y, z, dx, dy, dz, heading]
    ...
    # bev overlap
    overlaps_bev = torch.cuda.FloatTensor(torch.Size((boxes_a.shape[0],))).zero_()  # (N,)
    iou3d_nms_cuda.box_overlap_bev_gpu(boxes_a.contiguous(), boxes_b.contiguous(), overlaps_bev)

    max_of_min = torch.maximum(boxes_a_height_min, boxes_b_height_min)
    min_of_max = torch.minimum(boxes_a_height_max, boxes_b_height_max)
    overlaps_h = torch.clamp(min_of_max - max_of_min, min=0)
    min_of_min = torch.minimum(boxes_a_height_min, boxes_b_height_min)
    max_of_max = torch.maximum(boxes_a_height_max, boxes_b_height_max)
    out_h = torch.clamp(max_of_max - min_of_min, min=0)

    # 3d iou
    overlaps_3d = overlaps_bev * overlaps_h

    vol_a = (boxes_a[:, 3] * boxes_a[:, 4] * boxes_a[:, 5]).abs()
    vol_b = (boxes_b[:, 3] * boxes_b[:, 4] * boxes_b[:, 5]).abs()
